$(document).ready(function() {
    ...
    manageProductTable = $('#manageProductTable').DataTable({
        'ajax': '/ipheya/core/sub/php_action/fetchProduct.php',
        'order': []
    });
    ...
    $("#addProductModalBtn").unbind('click').bind('click', function() {
        ...
        $("#submitProductForm").unbind('submit').bind('submit', function() {
            ...
            if (reorder == "") {
                $("#reorder").after('<p class="text-danger">Brand Name field is required</p>');
                $('#reorder').closest('.form-group').addClass('has-error');
            } else {
                // remov error text field
                $("#reorder").find('.text-danger').remove();
                // success out for form
                $("#reorder").closest('.form-group').addClass('has-success');
            } // /else
            ...
            if ((productImage && productName && reorder && brandName && categoryName && productStatus) !=null)
            {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                            ...
                            {
                              $('#add-product-messages').html('<div class="alert alert-error">' +
                                  '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                                  '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> ' + response.messages +
                                  '</div>');
                            }
    ...
    if (productId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                    ...
                    $("#getProductImage").attr('src', 'data:image; base64,' + response.product_image);
                    ...
                    $("#editProductImage").fileinput({
                       ...
                       elErrorContainer: '#kv-avatar-errors-1',
                       msgErrorClass: 'alert alert-block alert-danger',
                       defaultPreviewContent: '<img src="data:image; base64,'+ response.product_image +'" alt="Profile Image" style="width:100%;">',
                       layoutTemplates: {main2: '{preview} {remove} {browse}'},
                    		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
                    });
                    ...
                    // reorder
                    $("#editreorder").val(response.reorder);
                    // brand name
                    ...
                    $("#editProductForm").unbind('submit').bind('submit', function() {
                        ...
                        if (reorder == "") {
                            $("#editreorder").after('<p class="text-danger">Brand Name field is required</p>');
                            $('#editreorder').closest('.form-group').addClass('has-error');
                        } else {
                            // remov error text field
                            $("#editreorder").find('.text-danger').remove();
                            // success out for form
                            $("#editreorder").closest('.form-group').addClass('has-success');
                        } // /else
                        ...
                        if (productName && reorder && brandName && categoryName && productStatus) {
                            // submit loading button
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                        ...
                                        if (response.success == true) {
                                            ...
                                            $.ajax({
                                                url: '/ipheya/core/sub/php_action/fetchProductImageUrl.php?i=' + productId,
                                                type: 'post',
                                                success: function(response) {
...
function removeProduct(productId = null) {
    if (productId) {
        ...
        $("#removeProductBtn").unbind('click').bind('click', function() {
            ...
            $.ajax({
                url: '/ipheya/core/sub/php_action/removeProduct.php',
                type: 'post',
                data: { productId: productId },
                dataType: 'json',
                success: function(response) {
