...
$(document).ready(function() {
	...
	manageCategoriesTable = $('#manageCategoriesTable').DataTable({
		'ajax' : '/ipheya/core/sub/php_action/fetchCategories.php',
		'order': []
	...
	$('#addCategoriesModalBtn').unbind('click').bind('click', function() {
		...
		$("#submitCategoriesForm").unbind('submit').bind('submit', function() {
			...
			if(categoriesStatus == "") {
				$("#categoriesStatus").after('<p class="text-danger">Please select a status</p>');
...
function editCategories(categoriesId = null) {
	if(categoriesId) {
		...
		$.ajax({
			url: '/ipheya/core/sub/php_action/fetchSelectedCategories.php',
			type: 'post',
			...
			success:function(response) {
				...
				$("#editCategoriesForm").unbind('submit').bind('submit', function() {
					...
					if(categoriesStatus == "") {
						$("#editCategoriesStatus").after('<p class="text-danger">Please select a category status</p>');
	...
	$.ajax({
		...
		success:function(response) {
			...
			$("#removeCategoriesBtn").unbind('click').bind('click', function() {
				...
				$.ajax({
					url: '/ipheya/core/sub/php_action/removeCategories.php',
					type: 'post',
					data: {categoriesId: categoriesId},
					dataType: 'json',
					success:function(response) {
